...
om=0
filter='None'
cmd+=("src/phylDiagHomologyMatrixViewer.py data/genesST.${S1}.list.bz2 data/genesST.${S2}.list.bz2 data/ancGenes.${A}.list.bz2 ${C1}:${R1} ${C2}:${R2} ${ritb}mode:chromosomesRewrittenInTbs -filterType=${filter} -tandemGapMax=${tgm} -distanceMetric=${dm} -gapMax=${gm} -verbose -pThreshold=${pt} ${ibwg}identifyBreakpointsWithinGaps ${no}nonOverlappingSbs -overlapMax=${om} -out:ImageName=res/processingSteps/MH_0.svg -out:SyntenyBlocks=res/processingSteps/MH_0.txt -scaleFactorRectangles=${sffr}")

# tandemGapMax filtering InFamilies
filter='InBothGenomes'
for tgm in 0 10
do
	cmd+=("src/phylDiagHomologyMatrixViewer.py data/genesST.${S1}.list.bz2 data/genesST.${S2}.list.bz2 data/ancGenes.${A}.list.bz2 ${C1}:${R1} ${C2}:${R2} ${ritb}mode:chromosomesRewrittenInTbs -filterType=${filter} -tandemGapMax=${tgm} -distanceMetric=${dm} -gapMax=${gm} -verbose -pThreshold=${pt} ${ibwg}identifyBreakpointsWithinGaps ${no}nonOverlappingSbs -overlapMax=${om} -out:ImageName=res/processingSteps/MH_InFamilies_tgm${tgm}.svg -out:SyntenyBlocks=res/processingSteps/MH_InFamilies_tgm${tgm}.txt -scaleFactorRectangles=${sffr}")
done

# tandemGapMax filtering InBothGenomes
#filter='InBothGenomes'
#tgm=10
#cmd+=("src/phylDiagHomologyMatrixViewer.py data/genesST.${S1}.list.bz2 data/genesST.${S2}.list.bz2 data/ancGenes.${A}.list.bz2 ${C1}:${R1} ${C2}:${R2} ${ritb}mode:chromosomesRewrittenInTbs -filterType=${filter} -tandemGapMax=${tgm} -distanceMetric=${dm} -gapMax=${gm} -verbose -pThreshold=${pt} ${ibwg}identifyBreakpointsWithinGaps ${no}nonOverlappingSbs -overlapMax=${om} -out:ImageName=res/processingSteps/MH_InBothGenomes_tgm${tgm}.svg -out:SyntenyBlocks=res/processingSteps/MH_InBothGenomes_tgm${tgm}.txt -scaleFactorRectangles=${sffr}")
