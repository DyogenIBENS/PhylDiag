...
commandLines+=(
"python -O ../phylDiag.py data/genesST.${S1}.list.bz2 data/genesST.${S2}.list.bz2 data/ancGenes.${A}.list.bz2 -filterType=${f} -tandemGapMax=${tgm} -distanceMetric=${dm} -gapMax=${gm} -pThreshold=${pt} -gapMaxMicroInv=${gmmi} ${ibwg}identifyBreakpointsWithinGaps -overlapMax=${om} -verbose > res/${S1}_${S2}_Tgm${tgm}gM${gm}Gmmi${gmmi}IbwgOm${om}.sbs 2> >(tee logErr_syntenyBlocksDrawer.txt >&2)"
)
done
